<template>
	<div
		style="gap: 10px"
		class="flex flex-col w-full justify-center items-center mt-10"
	>
		...
		<div
			v-show="isLottie"
			id="lottie"
			style="position: fixed; width: 100vw; height: 100vh; top: 50px"
		></div>
...
<script setup lang="ts">
		...
		return new Promise<void>(async (res) => {
			...
			for (let i = 0; i < values.length; i++) {
				...
				const el: any = document.querySelector<HTMLElement>(
					`#row-${activeRow.value}-${i}`
				);
				let k: any = document.getElementById(
					`key-${rows[activeRow.value].value[i].toLowerCase()}`
				);
	...
	const onRestart = () => {
		...
		document.querySelectorAll(".animate__animated").forEach((e) => {
	...
	const firework = () => {
		...
		Lottie.loadAnimation({
			...
			loop: true,
			autoplay: true,
			path: "https://assets8.lottiefiles.com/packages/lf20_dwm2hi59.json",
		});
	};
	const listenKey = () => {
		document
